...
CFLAGS += -ggdb
# make xv6 halt on panic
CFLAGS += -DHALT_ON_PANIC
...
QEMUOPTS := \
  -drive file=xv6.img,index=0,media=disk,format=raw \
	...
	-drive file=test.img,index=2,media=disk,format=raw \
  -smp $(CPUS) \
...
CLEAN := $(KERNEL_CLEAN) $(USER_CLEAN) $(TOOLS_CLEAN) \
	fs fs.img test.img kernel.asm .gdbinit .bochsrc dist
...
fs/README.md: README.md | fs
	cp $< $@

USER_BINS := $(notdir $(USER_PROGS))
fs.img: tools/mkfs fs/README.md $(addprefix fs/,$(USER_BINS))
	./tools/mkfs fs.img fs

test.img: testfs/README
	./tools/mkfs test.img testfs
